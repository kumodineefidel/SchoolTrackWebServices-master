...
public class RouteList {
	private final static String USER_AGENT = "Mozilla/5.0";
		...
		post.setHeader("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8");
			...
			System.out.println("Is Valid:"+request.isRequestedSessionIdValid());
			Login lg = new Login();
			...
			try{
			    ...
			    String sql  =  "select * from GeoCorridorList";
			    Query query = session.createSQLQuery(sql);
			    System.out.println(query.list().toString());
			    String[] corridorList = new String[query.list().size()];
			    int i=0;
			    Map<String,Object> map1 = new HashMap<String, Object>();
			    for (Object rows : query.list()) {
				    Object[] row = (Object[]) rows;
				    String corridorId = (String) row[1];
				    corridorList[i] = corridorId;
				    map1.put(corridorId, row[3]);
				    i++;
				}
			    ...
			    for (String string : corridorList) {
					System.out.println("corridorId"+string);
				}
			    
			    Session session1 = schoolTrackSessionFactory.openSession();
			    String sql1  =  "select * from route where ";
			    Query query1 = session1.createSQLQuery(sql1);
			    ...
			    for (Object rows : query1.list()) {
				    ...
				    String routeId = (String) row[2];
				    routeList[k] = routeId;
				    map.put(routeId, row[3]);
				    k++;
				}
			    
			    for (String string : routeList) {
					System.out.println("routeList"+string);
				}
			    
			    
			    System.out.println("Common Route:");
			    String foundId = null;
			    for(int l=0;l<corridorList.length;l++){
		            for(int m=0;m<routeList.length;m++){
		            	System.out.println("corridorList[l]"+corridorList[l]);
		            	System.out.println("routeList[m]"+routeList[m]);
		                if(corridorList[l].equals(routeList[m])){
		                    System.out.println("matched"+corridorList[l]);
		                     foundId = corridorList[l];
		                }
		            }
		            
		        }
			    System.out.println("MAP:"+map.toString());
			    Object value = map.get(foundId);
	            if(value!=null){
	            	System.out.println("\nIn Map"+value.toString());
